...
type Grapher struct {
	...
	SequenceTemplates map[string]*TemplateGraph // Template graphs for (valid) sequences in AllSequences
...
func NewGrapher(specs spec.Specs, idGenFactory id.GeneratorFactory, logFunc func(string, ...interface{})) *Grapher {
	return &Grapher{
		...
		LogFunc:           logFunc,
		SequenceTemplates: make(map[string]*TemplateGraph),
...
func (o *Grapher) getTemplate(seq *spec.SequenceSpec, jobArgs map[string]bool) (*TemplateGraph, error) {
	// Generates IDs unique within the template
	idgen := o.IdGenFactory.Make()

	template, err := NewTemplateGraph(seq.Name, idgen)
	if err != nil {
		return nil, err
	}
	g := &template.Graph
