...
type Runner interface {
	// Run runs the job, blocking until it has completed or until Stop is called.
	// It returns true if the job completes, else false. Jobs are all or nothing,
	// so "completes" means the returns on its own (isn't stopped) with its
	// state set to STATE_COMPLETE. jobData from the previous job is passed to
	// the job, and the job is free to write to it.
